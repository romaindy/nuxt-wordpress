...
import normalizeYoast from "../utils/normalizeYoast.js";
export const useYoastSEO = (source) => {
  const yoast = source && "yoast_head_json" in source && source.yoast_head_json ? normalizeYoast(source.yoast_head_json) : normalizeYoast(source) ?? void 0;
  if (!yoast) return;
  const meta = [];
  ...
  useHead({
    title: yoast.title,
    link: yoast.canonical ? [{ rel: "canonical", href: yoast.canonical }] : [],
    meta
  });
};
